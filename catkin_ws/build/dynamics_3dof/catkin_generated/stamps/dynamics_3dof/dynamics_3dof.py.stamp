...
def compute_left_inverse(matrix):
    left_inverse_matrix = np.dot(np.linalg.inv(np.dot(A.T, A)), A.T)
...
FREE_FLYER_BLADE_MAX_RPS = 200 # ACROBAT propellers rotations per second
...
current_position = np.array((1, 2, 0))
current_attitude = np.array((0, 0, np.pi/2)) # Euler angles as x, y, z

def input_objective_pose():
    position = input("Enter desired position (x, y): ")
    attitude = input("Enter desired attitude (z) euler angles: ")
    attitude = (0, 0, attitude)
